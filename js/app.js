...
const el = {
    ...
    knowledgeRoot: document.getElementById('knowledgeRoot')
};
...
function populateYears(preserveYear) {
    const prev = preserveYear != null && preserveYear !== '' ? String(preserveYear) : el.birthYear.value || '';
    ...
    for (let y = YEAR_END; y >= YEAR_SELECT_START; y--) {
        ...
        if (currentCalendar === 'western') {
            ...
            opt.textContent = roc > 0 ? `民國 ${roc} 年（${y}）` : `${y} 年`;
        }
...
function initTabs() {
    ...
    const available = Array.from(el.tabBtns).some(btn => btn.dataset.tab === requested);
    switchTab(available ? requested : 'paipan');
}
...
function switchTab(tabId) {
    ...
    if (tabId === 'knowledge') {
        renderKnowledge(el.knowledgeRoot);
    }
}
...
function handleSubmit(e) {
    ...
    saveState({
        ...
        advanced: el.showAdvanced.checked,
...
function runCalculation(westernYear, month, day, hour, gender) {
    ...
    setTimeout(() => {
        const result = safeExecute(() => calculateBazi(westernYear, month, day, hour, gender), {
        ...
        // 預先運算格局，供快速切換（失敗僅記錄）
        safeExecute(() => appStore.setGejuResult(determineGeju(result)), {
...
function renderFavoriteElement(result) {
    ...
    const favHTML = fe.favorite
        .map(el => `<span class="${ELEMENT_COLORS[el]}" style="font-size:1.8rem;font-weight:700;">${el}</span>`)
        .join(' ');

    const unfavHTML = fe.unfavorite
        .map(
            el =>
...
function renderLuck(result) {
    ...
    let html = `
            <div class="luck-overview">
                <span>起運 <strong>${gl.startAgeExact ?? gl.startAge} 歲</strong></span>
                <span>約 ${gl.startYear} 年</span>
                <span>${gl.isForward ? '順行' : '逆行'} · ${gl.isYangYear ? '陽' : '陰'}年</span>
                ${gl.targetTerm ? `<span>交運節氣 ${gl.targetTerm}</span>` : ''}
            </div>
            <p class="luck-note">大運由月柱依順逆方向逐柱排列，每柱約主十年環境。下列喜忌與互動須再配合各年流年，不宜單憑一柱定吉凶。</p>
            <div class="luck-cycle-grid">
        `;

    gl.luckCycles.forEach(luck => {
        const interactions = luck.interactions?.length
            ? luck.interactions
                  .slice(0, 2)
                  .map(item => `<span class="luck-tag luck-${item.type}">${item.label}</span>`)
                  .join('')
            : '<span class="luck-tag">平</span>';
        html += `
                <article class="luck-cycle-card" data-favor="${luck.favor || '中'}">
                    <div class="luck-cycle-top">
                        <span class="luck-index">第 ${luck.index} 運</span>
                        <span class="luck-favor">${luck.favor || '中'}</span>
                    </div>
                    <div class="luck-ganzhi">
                        <span class="${ELEMENT_COLORS[luck.stemElement] || ''}">${luck.stem || '—'}</span><span class="${ELEMENT_COLORS[luck.branchElement] || ''}">${luck.branch || '—'}</span>
                    </div>
                    <div class="luck-meta">${luck.ageStart}–${luck.ageEnd} 歲 · ${luck.startYear}–${luck.endYear}</div>
                    <div class="luck-ten-god">${luck.tenGod || luck.name || '十神未定'}</div>
                    <div class="luck-tags">${interactions}</div>
                    <p>${luck.analysis || ''}</p>
                </article>
            `;
    });
