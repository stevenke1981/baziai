...
/** 天干五行：index → element */
export const STEM_ELEMENT = ['木', '木', '火', '火', '土', '土', '金', '金', '水', '水'];
 ...
 * @returns {Object}
 */
export function calculateGreatLuck(birthYear, birthMonth, birthDay, hour, gender, yearStemIndex) {
    ...
    // 大運每十年一柱，最多排8柱（80年）
    const luckCycles = [];
    const tenGodCycle = isForward
        ? ['正財', '偏財', '正官', '七殺', '正印', '偏印', '比肩', '劫財', '食神', '傷官']
        : ['傷官', '食神', '劫財', '比肩', '偏印', '正印', '七殺', '正官', '偏財', '正財'];
    ...
    for (let i = 0; i < 8; i++) {
        ...
        luckCycles.push({
            ...
            name: tenGodCycle[i % tenGodCycle.length]
        });
    }
...
export function calculateBazi(year, month, day, hour = null, gender = null) {
    ...
    if (gender) {
        result.greatLuck = calculateGreatLuck(
            year, month, day, hour || 12, gender, 
            yearPillar.stemIndex, yearPillar.branchIndex
        );
    }
...
export function generateSummary(baziResult) {
    ...
    lines.push(`出生：${baziResult.input.year}年${baziResult.input.month}月${baziResult.input.day}日` +
...
if (typeof window !== 'undefined') {
    const exports = {
        STEMS, BRANCHES, STEM_ELEMENT, STEM_YIN_YANG,
        BRANCH_ELEMENT, BRANCH_YIN_YANG, HIDDEN_STEMS,
        ZODIAC, NAYIN, ELEMENT_COLORS, ELEMENT_CYCLE, HOUR_PERIODS,
        getLichunDate, getYearPillar, getSolarTerms,
        getApproximateSolarTermDate, getMonthBranchIndex, getMonthPillar,
        daysFrom19000101, getDayPillar, getHourBranch, getHourPillar,
        getElementRelation, getTenGod, getHiddenStemsWithTenGod, getNayin,
        calculateElementStrength, getFavoriteElement, getSolarTermDates,
        calculateGreatLuck, calculateBazi, generateSummary,
        rocToWestern, westernToRoc, isValidDate
    };
